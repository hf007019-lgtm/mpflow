...
请把下面规则追加到 CLAUDE.md，作为以后本项目的 Git 提交规则。
...
For business code changes, Claude must not auto-push immediately. It must:
1. show git status
...
For deployment-sensitive files, Claude must ask for confirmation before commit/push, including:
- docker-compose.yml
- Dockerfile
- nginx.conf
...
If git push fails because of network or authentication, Claude should stop and tell me the exact error.
